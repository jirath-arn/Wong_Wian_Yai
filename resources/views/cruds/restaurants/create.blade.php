...
@section('head')
    <title>{{ trans('panel.page_') }}{{ trans('global.create') }} {{ trans('cruds.restaurant.title_singular') }}</title>
@endsection
...
<section id="hero" class="d-flex align-items-center">
    <div class="container text-center position-relative" data-aos="fade-in" data-aos-delay="200">
        <h1>{{ trans('global.create') }} {{ trans('cruds.restaurant.title_singular') }}</h1>
            ...
            <div class="col-lg-9">
                <div class="col-md-12 align-items-stretch mt-4">
                    <div class="icon-box" data-aos="zoom-in" data-aos-delay="50">
                        <h2 style="text-align: center;"><a href="#restaurants">{{ trans('global.create') }} {{ trans('cruds.restaurant.title_singular') }}</a></h2><br>
                                    ...
                                    <div class="form-group {{ $errors->has('categories') ? 'has-error' : '' }}">
                                        ...
                                        <select name="categories" id="categories" class="form-control select2" required>
                                            @foreach($categories as $id => $categories)
                                                <option value="{{ $id }}" {{ (in_array($id, old('categories', [])) || isset($restaurant) && $restaurant->category->contains($id)) ? 'selected' : '' }}>{{ $categories }}</option>
                                            @endforeach
                                        </select>
